...
function updateToolTip(chosenXAxis, circlesGroup) {
  ...
  if (chosenXAxis === "beer_ibu") {
    label = "Beer Bitterness:";
  }
  else {
    label = "Beer Alcohol Content:";
  }
...
d3.json(beerDbUrl).then(function(beerData, err) {
  if (err) throw err;
  ...
  var circlesGroup = chartGroup.selectAll("circle")
    ...
    .attr("cy", d => yLinearScale(d.beer_id))
    .attr("r", 5)
    .attr("fill", "yellow")
    .attr("opacity", ".5");
  ...
  var hairLengthLabel = labelsGroup.append("text")
    .attr("x", 0)
    .attr("y", 20)
    .attr("value", "beer_ibu") // value to grab for event listener
    .classed("active", true)
    .text("Beer Bitterness");

  var albumsLabel = labelsGroup.append("text")
    .attr("x", 0)
    .attr("y", 40)
    .attr("value", "beer_abv") // value to grab for event listener
