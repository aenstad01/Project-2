...
d3.csv("resources/data/craftbeerdotcom_beer_styles_for_table.csv").then(function (data, err) {
  if (err) throw err;

  var beer_tbody = d3.select("#beertbody");
  var beer_thead = d3.select("#beerthead");

  var tableData = data;
  function fillBeerTable(x) {
    //Put in header rows 
    let labelrow = beer_thead.append("tr");
    Object.entries(tableData[0]).forEach(([title,]) => {
      let cell = labelrow.append("td");
      cell.text(title);
    });
    //Add rows and fill them with data (assuming all data organized the same way)
    x.forEach((beer) => {
      let row = beer_tbody.append("tr");
      Object.entries(beer).forEach(([, value]) => {
    ...
    var chartdata0 = [
      {
        ...
        theta: ['Original gravity<br> (max: ' + maxDataA + ')', 'Final gravity<br> (max: ' + maxDataB + ')', 'Alcohol by volume<br> (max: ' + maxDataC + ')', 'International<br> bitterness units<br> (max: ' + maxDataD + ')', 'Bitterness ratio<br> (max: ' + maxDataE + ')', 'Standard reference<br> method<br> (max:' + maxDataF + ')', 'Volumes of CO2<br> (max:' + maxDataG + ')', 'Apparent attenuation<br> (max:' + maxDataH + ')', 'Original gravity<br> (max: ' + maxDataA + ')'],
        name: "Maximum",
        line: {
          color: "darkgoldenrod"
        },
